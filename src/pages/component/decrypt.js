...
const Decrypt = () => {
  ...
  const getBalanceSelect = async () => {
    try {
      setSelectedItemBalance(assetItem?.balance);
      let res = await axios.get(
        `${Api_URL}/transactions/get-price?coinId=${assetItem.id}`
      );
      if (res) {
  ...
  const Decrypt = async (e) => {
    ...
    if (account) {
      ...
      try {
        if (Web3.utils.isAddress(exAccount)) {
          if (assetItem) {
            let amou = parseFloat(amount);
            if (assetItem?.balance >= amou && amou > 0) {
              const key = "dopTPL_";
              const myPassword = CryptoJS.AES.decrypt(
                current?.password,
                key
              ).toString(CryptoJS.enc.Utf8);
              if (password === myPassword) {
                setLoader(true);
                setLoadMsg("Preparing to decrypt...");
                ...
                // let res = await axios.get(
                //   `${CoingekoBaseURL}/coins/${assetItem.id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false&x_cg_pro_api_key=${apiKey}`
                // );
                if (res) {
                  dollarPrice =
                    res?.data?.data?.market_data?.current_price?.usd;
                  let decimals = await DecimalPoints(assetItem?.address);
                  let decs = 18;
                  if (decimals === 6n) {
                    decs = 6;
                  } else if (decimals === 9n) {
                  ...
                  const tomiAmountToUnShield = BigInt(
                    amou * Math.pow(10, decs)
                  );
                  const erc20AmountRecipients = [
                    {
                      tokenAddress: assetItem?.address, // TOMI
                      amount: tomiAmountToUnShield, // hexadecimal amount
                      recipientAddress: exAccount,
                    },
                  ];
                  ...
                  const selectedTokenFeeAddress =
                    "0x49630C4E508ab3Fd339905424CE33D9fBa8FBABF";
                  const key = "dopTPL_";
              ...
              if (amou === 0) {
                setError("Set amount at least greater than 0.");
              } else {
                setError("Amount cannot be greater than balance.");
              }
            }
          } else {
            setError("Select asset that needs to be send!");
          }
        } else {
          setError("Not a valid web3 account.");
        }
      } catch (err) {
        setLoader(false);
        console.log("err !!", err);
  ...
  const PopulateTransaction = async (
    ...
    if (populateResponse?.transaction) {
      try {
        ...
        let currentTransactions = await getData("decTransactions");
        ...
        if (transactionHash) {
          ...
          axios
            .post(`${Api_URL}/transactions/decrypt`, postData, {
              headers: {
            ...
            .then(async (response) => {
              ...
              setLoader(false);
              setLoadMsg("");
              await saveData("decTransactions", currentTransactions);
            })
            .catch(async (error) => {
  ...
  const OpenConfirm = () => {
    if (parseFloat(amount) <= parseFloat(assetItem?.balance)) {
      setShow2(true);
    } else {
      if (parseFloat(amount) > 0) {
        setError("Decrypt amount cannot be greater than balance!");
      } else {
          ...
          <div className="encrypt-parent">
            ...
            <div className="bottom-content">
              ...
              >
                ...
                {assetItem ? (
                  <div className="left-side">
                    <img src={assetItem?.igm} alt="img" className="img-fluid" />
                    <p className="set-text-after-select">{assetItem?.name}</p>
                  </div>
                ) : (
                  <div className="left-side">
                    {/* <img
                ...
                <div className="right-side d-flex">
                  {assetItem && (
                    <div className="with-balance">
                      {selectedItemBalance > 0.0 ? (
                        <h6>
                          Balance:{" "}
                          {parseFloat(selectedItemBalance)
                            ?.toString()
                            ?.slice(0, 4)}
                        </h6>
                      ) : (
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    <path d="M14.3045 7H10.2997H6.69346C6.07635 7 5.76779 7.75891 6.20492 8.20379L9.53476 11.5927C10.0683 12.1358 10.9361 12.1358 11.4697 11.5927L12.736 10.3039L14.7995 8.20379C15.2302 7.75891 14.9217 7 14.3045 7Z" fill="white" />
                  </svg>
                </div>
              </div>
              <div className="amount-div" style={{ marginBottom: "30px" }}>
                ...
                <a
                  onClick={() => setAmount(assetItem?.balance)}
                  className="btn-max"
                >
                  MAX
                </a>
              ...
              <div class="material-textfield">
                <input
                  value={exAccount}
                  onChange={(e) => setExAccount(e.target.value?.trim())}
              ...
              <div class="material-textfield">
                <input
                  ...
                  placeholder="Enter Your DOP Wallet Password"
                />
      ...
      >
        ...
        <Modal.Body>
          <div className="bottom-content">
            ...
            <div className="inner-text">
              <p>Asset</p>
              <h6>{assetItem?.name}</h6>
            </div>
            <div className="inner-text">
              <p>Recipient Address</p>
              <h6>
                {exAccount?.slice(0, 12)}...
                {exAccount?.slice(exAccount?.length - 6, exAccount?.length)}
              </h6>
            </div>
            <div className="inner-text">
              <p>Fee</p>
              <h6>
                {((0.1 / 100) * amount)?.toFixed(6)} {assetItem?.name}
              </h6>
            </div>
            <div className="inner-text">
              <p>Amount</p>
              <h6>{amount + " " + assetItem?.name}</h6>
            </div>
          </div>
          <div className="total-value">
            <div className="inner-text">
              ...
              <h6>
                ...
                {assetItem?.name}
                {/* {amount + (5 / 100) * amount} {assetItem?.name} */}
                <br />
                {/* {(5 / 100) * amount} {assetItem?.name} */}
              </h6>
            </div>
          </div>
      ...
      >
        <Modal.Body>
          <div className="inner-content">
            ...
            />
            <h6 className="main-head">Successfully Decrypted</h6>
      ...
      >
        <Modal.Body>
          <div className="badges-content desktop-view-badge">
            ...
            <img src="\assets\dummy-badge.svg" alt="img" className="img-fluid" />
            <h5>Encrypt $10,000</h5>
            <a href="#" className="btn-badge">Go to badges</a>
          </div>
          <div className="badges-content mobile-view-badge d-none">
            <img src="\assets\dummy-badge.svg" alt="img" className="img-fluid" />
            <h6>Congratulations</h6>
            <p>You have successfully got <span> Encrypt $10,000</span> Badge</p>
            <a href="#" className="btn-badge">Go to badges</a>
          </div>
        </Modal.Body>
      </Modal>
